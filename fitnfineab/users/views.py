...
from users.forms import AuthenticationForm, FitnessMeterForm, NewUserCreationForm, NewUserChangeForm, WeightMetricForm, HeightMetricForm, WeightTrackForm, ContactForm
...
def signup(request):
	if request.method == "POST":
		...
		if form.is_valid():
			user = form.save()
			login(request, user)
			messages.success(request, "Registration successful. Welcome to Fitnfine. You are now a member of the family.")
			return redirect("users:login")
...
def fitnessMeter(request):
	if request.method=="POST":
		...
		if form.is_valid():
			...
			return render(request, "users/result.html", {'name':name,'bmi':bmi, 'whr':whr,'weightShouldBe_one':weightShouldBe_one,
								'weightShouldBe_two':weightShouldBe_two, 'gender':gender,
								'min_weight_loss':min_weight_loss} )
	form=FitnessMeterForm()
	return render(request, "users/fitnessMeter.html", {'form':form})	
...
def profile(request):
	...
	if request.method=="POST":
		...
		if form.is_valid():
			...
			return render(request, "users/userProfile.html", {'bmi':bmi, 'whr':whr, 'user_stats': user_stats})
	...
	waist=u.waist
	hips=u.hips
	return render(request, "users/profile.html", {'form':form, 'weight':weight, 'height':height, 'waist':waist, 'hips':hips })

def m_converter(request):
	return render(request, "users/m_converter.html")
...
def metric_converter(request):
	if request.method=="POST":
		...
		if weight_form.is_valid():
			...
			return render(request, "users/metric_converter.html", context)
	weight_form = WeightMetricForm()
	
	return render(request, "users/metric_converter.html", {"weight_form":weight_form})
...
def metric_converter_two(request):
	if request.method=="POST":
		...
		if height_form.is_valid():		
			...
			return render(request, "users/metric_converter_two.html", {"height_in_meter":height_in_meter, "height_in_cm":height_in_cm })
	height_form = HeightMetricForm()
	
	return render(request, "users/metric_converter_two.html", {"height_form":height_form})
...
def weight_tracker(request):
	...
	if request.method=="POST":
		...
		if form.is_valid():
			...
			return render(request, "users/weight_tracker.html", {'min_time_to_get_result_days':min_time_to_get_result_days, 'max_time_to_get_result_days':max_time_to_get_result_days, 'weight':weight, 'desired_weight':desired_weight, 'weightShouldBe_one':weightShouldBe_one, 'min_month':min_month, 'days_left':days_left})		
	return render(request, "users/weight_tracker.html", {'form': form, 'weight':weight})
